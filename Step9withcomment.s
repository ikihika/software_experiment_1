...
timer_interrupt:
	...
	move.w TSTAT1, %d0		  /* TSTAT1の第0ビットが1となっているかどうかをチェック  */
	andi.w #0x0001,%d0                  
	cmpi.w #0x0001,%d0		  
	bne  end_interrupt1		  /* 0 ならば rte で復帰 */

	move.l #0x00000000, TSTAT1	  /* TSTAT1をリセット */
	jsr CALL_RP
...
*****************************************************************************************
** タイマ制御
** RESET_TIMER:タイマ割り込みを不可にし，タイマも停止する．
**             入出力なし	
** SET_TIMER:タイマ割り込み時に呼び出すべきルーチンを設定する
**           タイマ割り込み周期 t を設定し t * 0.1 msec 秒毎に割り込みが発生するようにする．
**           入力･･･タイマ割り込み発生周期 t → %d1.w (型に注意)
**                  割り込み時に起動するルーチンの先頭アドレス p → %d2.L
** CALL_RP:タイマ割り込み時に処理すべきルーチンを呼び出す．
**         入出力なし	
*****************************************************************************************
	.section .text
...
SET_TIMER:
	move.b #'3',LED5
	movem.l %d1-%d2/%a0, -(%sp)
	move.l %d2,task_p		  /* 割り込み時に起動するルーチン先頭アドレス p を */
					  /* 大域変数 task p に代入 */

	move.w #0x00ce, TPRER1		  /* 0.1 msec 進むとカウンタが 1 増えるように設定 */

	move.w %d1, TCMP1		  /* タイマ割り込み発生周期 t を， TCMP1 に代入  */
...
CALL_RP:
	...
	movem.l %a0, -(%sp)
	movea.l  task_p,%a0
	jsr (%a0)			  /* 大域変数 task p の指すアドレスへジャンプ */
	movem.l (%sp)+, %a0
	rts
...
SYSTEM_CALL:
	...
	lea.l GETSTRING, %a0		  /* a0にGETSTRINGのアドレスを格納  */
	subi.l #1,%d0			  /* システムコール番号が 1 (GETSTRING) か確認 */
	beq CALL_Finish

	lea.l PUTSTRING, %a0		  /* a0にPUTSTRINGのアドレスを格納  */
	subi.l #1,%d0			  /* システムコール番号が 2 (PUTSTRING) か確認 */
	beq CALL_Finish
	
	lea.l RESET_TIMER, %a0		  /* a0にRESET_TIMERのアドレスを格納  */
	subi.l #1,%d0			  /* システムコール番号が 3 (RESET_TIMER) か確認 */
	beq CALL_Finish

	lea.l SET_TIMER, %a0		  /* a0にSET_TIMERのアドレスを格納  */

CALL_Finish:
	jsr (%a0)			  /* a0に格納されているアドレスに飛ぶ  */
	movem.l (%sp)+,%a0
	rte	
...
************************************************************************************
**PUTSTRING(ch, p, size)
**入力:チャネルch(%d1.l), 読み込み先の先頭アドレスp(%d2.l), 送信するデータ数size(%d3.l)
...
PUT_LOOP:
	...
	cmp.l  %d4, %d3      
	beq    UNMASK         		  /*sz = sizeのときアンマスク*/
	move.l #1, %d0        		  /*送信キューを指定*/
	move.b (%a1), %d1     		  /*送信するデータを指定*/
	jsr    INQ
	cmpi.l #0, %d0     
	beq    UNMASK         		  /*INQが失敗のとき*/
