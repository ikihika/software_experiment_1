...
***********************************************
** エコーバックおよびタイマー割込みのプログラム
** エコーバックを行う。
** さらに5秒に1回"******"という文字列を表示し改行する(最大5回まで)。
** 入力：キーボードによる入力
** 出力：入力された文字
**	5秒おきに入力される文字列"******"と改行
***********************************************
...
RESET_TIMER:
	move.w #0x0004, TCTL1 /*TCTL1 を設定 (restart, 割り込み不可，システムクロックの 1/16 を単位として計時，タイマ使用停止)．*/
	rts
...
SET_TIMER:
	move.l %d2, task_p /*先頭アドレスpを，大域変数task-pに代入*/
	move.w #0x00ce, TPRER1 /*TPRER1を設定し，0.1msec進むとカウンタが 1 増えるようにする．*/
	move.w %d1, TCMP1 /*タイマ割り込み発生周期tを，タイマ1コンペアレジスタ TCMP1 に代入する．*/
	move.w #0x0015, TCTL1 /*TCTL1 を設定 (restart, 割り込み許可 (enable the compareinterrupt)，システムクロックの 1/16 を単位として計時，タイマ使用許可) し，
                               タイマをスタートさせる*/
	rts
